...
name='delete'dept_id='delete'
action='".htmlspecialchars($_SERVER["PHP_SELF"])."'
method='POST'><input type='hidden' value='".$row_id."'
name='row_id'dept_id='row_id'><input value='Delete' type='submit'
name='delete_button'dept_id='delete_button'></form>";
        return $btn;
    }
?>
...
echo "<tr><th>Department Id</th><th>Department Code</th><th>Department Name</th><th>Action</th></tr>";
...
if ($result->num_rows > 0) {
    ...
    // output data of each row
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row['dept_id']."</td><td>".$row['dept_code']."</td><td>".$row['dept_name']."</td><td>".delete_button($row['dept_id'])."</td></tr>";
            $i++;
        }
} else {
